          ...
          {activeProject ? (
            ...
            <div className="animate-in fade-in duration-500" style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 32, flexShrink: 0 }}>
                <div>
                  <h1 style={{ margin: 0, fontSize: 32, fontWeight: 900, color: 'var(--text-primary)', letterSpacing: '-0.03em' }}>项目中心</h1>
                  <p style={{ margin: '6px 0 0', fontSize: 14, color: 'var(--text-muted)', fontStyle: 'italic' }}>Communication Design Project Hub</p>
                </div>
                <button
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 24, flexShrink: 0 }}>
                <div style={{ position: 'relative' }}>
                  <Search size={18} style={{ position: 'absolute', left: 14, top: '50%', transform: 'translateY(-50%)', color: 'var(--text-faint)', pointerEvents: 'none' }} />
                  <input
                    type="text"
                ...
                <div style={{ display: 'flex', gap: 12 }}>
                  {[
                    { icon: <MapPin size={15} />, options: ['全部', ...metaHistory.cities], value: cityFilter, onChange: setCityFilter, placeholder: '全部地市' },
                    { icon: <Tag size={15} />, options: ['全部', ...metaHistory.types], value: typeFilter, onChange: setTypeFilter, placeholder: '所有类型' },
                  ].map((f, i) => (
                    <div key={i} style={{
                      flex: 1,
                      display: 'flex',
                      ...
                      <span style={{ color: 'var(--text-faint)', flexShrink: 0 }}>{f.icon}</span>
                      <select
                        value={f.value}
                      ...
                      >
                        {f.options.map(o => <option key={o} value={o}>{o === '全部' ? (i === 0 ? '全部地市' : '所有类型') : o}</option>)}
                      </select>
                    </div>
                  ))}
                  <button style={{
              ...
              <div className="custom-scrollbar" style={{ flex: 1, overflowY: 'auto', paddingRight: 4 }}>
                {filteredProjects.length > 0 ? (
                  ...
                  <div style={{
                    ...
                    <p style={{ fontSize: 18, opacity: 0.3, fontStyle: 'italic', margin: 0 }}>目前此处空空如也</p>
                  </div>
                )}
              </div>
            </div>
            ...
            <Suspense fallback={<div className="flex-1 flex items-center justify-center">Loading Ability Manager...</div>}>
              <AbilityManager />
            </Suspense>
          ) : activeTab === 'common' ? (
            <CommonInfoManager />
            ...
            <div style={{
              ...
              <LayoutGrid size={64} style={{ opacity: 0.1, marginBottom: 16 }} />
              <p style={{ opacity: 0.3, textTransform: 'uppercase', letterSpacing: '0.2em', margin: 0 }}>{activeTab} Module Under Development</p>
            </div>
          )}
        </main>
